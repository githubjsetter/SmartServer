...
public class ZxhovUploadHelper  {
	Category logger=Category.getInstance(ZxhovUploadHelper.class);
	boolean cancelflag = false;
	boolean ok = false;
	String errormessage = "";
	String hovname;

	public boolean uploadZxhovfile(String hovname,File zxhovzipfile){
		this.hovname=hovname;
		UploadprogressDialog progdlg=new UploadprogressDialog("上传专项HOV文件");
		progdlg.setMessage("上传文件"+zxhovzipfile.getName());
		progdlg.pack();
		UploadThread uploadthread=new UploadThread(zxhovzipfile,progdlg);
		uploadthread.start();
		...
		void uploadZxfile(File zxzipfile) throws Exception {
			...
			try {
				...
				while (!cancelflag && length > 0) {
					ClientRequest req = new ClientRequest("npdev:上传专项HOV");
