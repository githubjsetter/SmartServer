...
import com.smart.platform.util.KeyNamePair;

/**
 * Created by IntelliJ IDEA. User: Administrator Date: 2007-4-20 Time: 15:30:43
 * To change this template use File | Settings | File Templates.
 */
public class CCalendar extends CDialog implements ActionListener,
		MouseListener, ChangeListener, KeyListener {
	/**
	...
	public CCalendar(Frame frame, String title) {
		this(frame, title, null, 0);
	} // Calendar
	 ...
	 */
	public CCalendar(Frame frame, String title, Timestamp startTS,
			int displayType) {
		super(frame, title, true);
		log.info(startTS == null ? "null" : startTS.toString() + " - DT="
				+ displayType);
	...
	/** Logger */
	private static Category log = Category.getInstance(CCalendar.class);
	//
	private CPanel mainPanel = new CPanel();
	private CPanel monthPanel = new CPanel();
	private CComboBox cMonth = new CComboBox();
	private JSpinner cYear = new JSpinner(new SpinnerNumberModel(2000, 1900,
			2100, 1));
	...
	private JSpinner fMinute = new JSpinner(new MinuteModel(5)); // 5 minute
																	// snap size
	...
	protected void localScreenCenter() {
		...
		/*
		 * //最大取2/3 this.setPreferredSize(size);
		 */
	...
	private void jbInit() throws Exception {
		...
		bBack.setIcon(IconFactory.icleft); // <
		bBack.setMargin(new Insets(0, 0, 0, 0));
		bBack.addActionListener(this);
		bNext.setIcon(IconFactory.icright); // >
		...
		((JSpinner.DefaultEditor) fMinute.getEditor()).getTextField()
				.addKeyListener(this);
	...
	protected void processWindowEvent(WindowEvent e) {
		...
		if (e.getID() == WindowEvent.WINDOW_OPENED) {
			/*
			 * if (m_displayType == DisplayType.Time) fHour.requestFocus(); else
			 * if (m_today != null)
			 */
	...
	/***************************************************************************
	 * Load Data. - Years - Month names - Day Names
	 * 
	 * @param startTS
	 *            time stamp
	 */
	private void loadData(Timestamp startTS) {
		m_calendar = new GregorianCalendar(Locale.getDefault());
		if (startTS == null)
		...
		SimpleDateFormat formatDate = (SimpleDateFormat) DateFormat
				.getDateInstance(DateFormat.LONG, loc);
		// Short: h:mm a - HH:mm Long: h:mm:ss a z - HH:mm:ss z
		SimpleDateFormat formatTime = (SimpleDateFormat) DateFormat
				.getTimeInstance(DateFormat.SHORT, loc);
		...
		String[] days = formatDate.getDateFormatSymbols().getShortWeekdays(); // 0 is
																				// blank,
																				// 1 is
																				// Sunday
		 ...
		 * m_days[m_days.length-1].setText("今天");
		 * m_days[m_days.length-1].setToolTipText("今天"); // Cancel
		 * m_days[m_days.length-2].setBackground(Color.red);
		 * m_days[m_days.length-2].setText("关闭");
		 * m_days[m_days.length-2].setToolTipText("取消");
		 * 
		 */
		// Date/Time
		...
		// What to show
		/*
		 * timePanel.setVisible(m_displayType == DisplayType.DateTime ||
		 * m_displayType == DisplayType.Time);
	...
	private void setCalendar() {
		...
		m_calendar.set(m_currentYear, m_currentMonth - 1, 1); // Month is zero
																// based
		...
		Color colorbackground = new Color(0xEE, 0xF3, 0xF9);
		Color colorforeground = new Color(0, 0, 0);

		// Color coloractivebackground=Color.BLUE;
		// Color coloraativeforeground=Color.YELLOW;

		Color coloractivebackground = new Color(141, 141, 186);
		Color coloraativeforeground = Color.WHITE;

		int curDay = 1;
		for (int i = 0; i < m_days.length; i++) {
			if (i >= dayOne && i <= lastDate) {
				if (m_currentDay == curDay) {
					m_days[i].setBackground(coloractivebackground);
					m_days[i].setForeground(coloraativeforeground);
					m_today = m_days[i];
					m_today.requestFocus();
				} else {
					m_days[i].setBackground(colorbackground);
					m_days[i].setForeground(colorforeground);
				}
				...
				// m_days[i].setBackground(Color.LIGHT_GRAY);
				m_days[i].setBackground(colorbackground);
			}
		}
		...
		lTZ.setText(tz.getDisplayName(tz.inDaylightTime(m_calendar.getTime()),
				TimeZone.SHORT));

		// Update Calendar
		m_calendar.set(m_currentYear, m_currentMonth - 1, m_currentDay,
				m_current24Hour, m_currentMinute, 0);
		m_calendar.set(java.util.Calendar.MILLISECOND, 0);
	} // setCalendar

	/**
	 * Set Current Time from UI. - set m_current.. variables
	 */
	...
	/***************************************************************************
	 * Action Listener for Month/Year combo & dat buttons. - Double clicking on
	 * a date closes it - set m_current...
	 * 
	 * @param e
	 *            Event
	 */
		...
		if (e.getSource() == bOK) {
			...
			// we have a day
				...
				if (m_currentDay == m_lastDay
						&& currentClick - m_lastClick < 1000) // double click
																// 1 second
				{
					m_abort = false;
					dispose();
...
/**
 * Minute Spinner Model. Based on Number Model - uses snap size to determine
 * next value. Allows to manually set any ninute, but return even snap value
 * when spinner buttons are used.
 * 
 */
class MinuteModel extends SpinnerNumberModel {
	/**
	 * Constructor. Creates Integer Spinner with minimum=0, maximum=59,
	 * stepsize=1
	 * 
	 * @param snapSize
	 *            snap size
	...
	public static void main(String[] argv) {
		CCalendar f = new CCalendar(null, "日期", null, 0);
		f.pack();
		f.setVisible(true);
	}
