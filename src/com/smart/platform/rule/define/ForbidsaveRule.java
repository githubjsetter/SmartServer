...
public class ForbidsaveRule extends Rulebase {
	static String[] treatableruletypes;
	static {
		treatableruletypes = new String[] { "屏蔽保存" };
	}
	...
	public int process(Object caller) throws Exception {
		...
		if (getRuletype().equals("屏蔽保存")) {
			if (caller instanceof CSteModel) {
				...
				CStetoolbar tb=searchToolbar(rootpane);
				if(tb!=null){
					hideButton(tb,CMdeModel.ACTION_SAVE);
				}
