...
import com.smart.platform.rule.setup.StoreprocHov;

public class StoreprocRule  extends Rulebase {
	static protected String[] treatableruletypes = null;
	static {
		treatableruletypes = new String[] { "后处理存储过程" };
	}
	...
	static class SetupDialog extends RulesetupDialogbase {
		...
		SetupDialog(String expr) {
			super((Frame) null, "设置保存后调用的存储过程名称");
			this.expr = expr;
			createComponent();
			bindValue();
			localCenter();
	...
	public static void main(String[] argv) {
		...
		StoreprocRule.SetupDialog dlg = new StoreprocRule.SetupDialog(expr);
		dlg.pack();
		dlg.setVisible(true);
		if (dlg.getOk()) {
